        ...
        <div class="details">
            <div class="recentRents">
                <div class="cardHeader">
                    <h2>All bookings on that location</h2>
                </div>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                <?php
                $sql="SELECT cc.car_id,cc.pick_up,cc.drop_off,cc.totalPrice FROM rented_car cc,car c WHERE cc.car_id=c.ID AND  c.location_id='.$id.'";
                $cars = $connect->query($sql);
                if (!$cars) {
                    die("Invalid Query: " . $connect->error);
                }
